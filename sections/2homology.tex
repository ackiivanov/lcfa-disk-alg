...
\begin{remark}\label{rem:fin_equiv_natural}
    ...
    \end{equation}
    %
    as discussed in \cite[prop.4.1.1.8]{lurie_htt}, where the functor is the obvious one. Furthermore, \cite[\href{https://kerodon.net/tag/016H}{016H}]{lurie_kerodon} tells us that the coslice construction is functorial, which for our purposes defines the functors
    %
    \begin{align}
    ...
    \end{equation}
    %
    in $\mathsf{Fun}(\mathsf{Fun(\mathscr{B}, \mathscr{C})}, \cat)$. But this is obviously the case, given the description of the maps in \cref{eq:fin_equivs_cat}.
\end{remark}
