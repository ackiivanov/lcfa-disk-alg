...
\begin{example}
    The open cone $C(S^k)$ on the $k$-sphere is canonically identified with $\mathbb{R}^k$ with stratification given by the filtration $\{0\}\subset\mathbb{R}^k$ as in \Cref{7UEDP24}.
\end{example}
...
\begin{remark}
    \Cref{thm:decom_strat_man} is the analogue of handle decompositions for smooth manifolds. Namely, any compact, smooth manifold can be obtained by handle decomposition, and non-compact smooth manifolds are obtained by sequential colimits of finitary (including compact) smooth manifolds.
\end{remark}
