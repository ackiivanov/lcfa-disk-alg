...
{\color{red} This is the statement in the absolute case, needs to be adapted to the relative case.}
...
\begin{lemma}\label{lem:gluing_disk/M/e}
    ...
    %
    \begin{equation}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r]_{/M} \xrightarrow{(\disk_{/M})_{/-}} \mathsf{RFib}_{\disk_{/M}}) \xrightarrow{\ \ \simeq \ \ } (\disk_{/M})_{/-}
    \end{equation}
    of right fibrations over $\disk_{/M}$, which moreover, is an equivalence.
\end{lemma}
...
\begin{proof}
    ...
    \end{equation}
    %
    %Indeed, let $X \simeq y_{\mathscr{D}} (d)$ be a representable presheaf, then there is an equivalence of morphism spaces $\mathsf{Hom}_{\catC} (L_{y_{\mathscr{D}}} F (X), c) \simeq \mathsf{Hom}_{\mathsf{PShv}(\mathscr{D})} (X, T_F (c))$ given by 
    %
    %\begin{align}
    %    \mathsf{Hom}_{\catC} (L_{y_{\mathscr{D}}} F (X), c) &\simeq \mathsf{Hom}_{\catC} (F (d), c) \simeq \mathsf{Hom}_{\mathsf{PShv}(\catC)} (y_{\catC}(F(d)), y_{\catC}(c))\notag\\ &\simeq \mathsf{Hom}_{\mathsf{PShv}(\mathscr{D})} (F^*(y_{\catC}(F(d))), T_F(c)) \simeq \mathsf{Hom}_{\mathsf{PShv}(\mathscr{D})} (X, T_F(c)).
    %\end{align}
    %
    %In general, the Yoneda embedding $y_{\mathscr{D}}$ is a dense functor
...
\begin{proof}[Proof of {\Cref{prop:fh_disk_to_lcfa}}]
    ...
    With the Weiss cosheaf property being a statement about preserving colimits, and with $\int F$ being a left Kan extension we are exactly in the situation of \Cref{lem:Lan_preserve_colim}. Thus, we only need to show that functor $T: \mfld_{/M} \xrightarrow{} \mathsf{PShv} (\disk_{/M})$\footnote{This functor is related to the tangent classifier, with the differences being that we're now in the relative case, and we're not fully limiting to $\bsc$ but instead only to disks.} preserves Weiss colimits, namely that there is an equivalence
    %
    \begin{equation}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld_{/M} \xrightarrow{T} \mathsf{PShv} (\disk_{/M})) \simeq T(N \xhookrightarrow{} M),
    \end{equation}
    %
    where on the right-hand side we've used the fact that $\mathscr{U}$ is, in particular, a covering sieve. If we also append the straightening--unstraightening construction to $T$, we can write the desired equivalence as
    %
    \begin{equation}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld_{/M} \xrightarrow{ (\disk_{/M})_{/-}} \mathsf{RFib}_{\disk_{/M}}) \simeq (\disk_{/M})_{/e}.
    \end{equation}
    %
    \Cref{lem:gluing_disk/M/e} guarantees the existence of this equivalence.
\end{proof}
