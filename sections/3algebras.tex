...
\begin{proof}
    The morphism is determined by the universal property of the colimit with the component maps $\disk_{/U} \xrightarrow{} \disk_{/M}$ given from the open embeddings $U \xhookrightarrow{} M$. To show the equivalence, we follow a related strategy to the one laid out in part 2 of the proof of \cite[Prop.2.22]{af_primer}. By \cite[Def.2.2.3.2]{lurie_htt}, a morphism of right fibrations, as in the statement, is an equivalence if, and only if, it induces equivalences of spaces on the fibers over each object of the base. The objects of the base $\disk(\bstr)$ (e.g. acroding to \cite[Lem.2.21]{aft_fhstrat}) are lists $(i_{[U]} | [U] \in [\bstr])$ of integers, one for each isomorphism class of objects in $\bstr$. According to the same lemma, the fibers are given by the spaces
    %
    \begin{equation}
        (\disk_{/M})_{(i_{[U]} | [U])} \simeq \coprod_{[U]} \mathrm{Conf}_{i_{[U]}}(M_{[U]})_{\Sigma_{i_{[U]}}}.
    \end{equation}
    %
    Thus, we endeavor to show that the induced morphism
    %
    \begin{equation}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\mathrm{Conf}_{i_{[U]}}(-_{[U]})_{\Sigma_{i_{[U]}}}} \mathsf{Sp}) \xrightarrow{\ \ \ \ \ } \mathrm{Conf}_{i_{[U]}}(M_{[U]})_{\Sigma_{i_{[U]}}}
    \end{equation}
    %
    is an equivalence of spaces for all isomorphism classes of basics $[U]$ and for all $i_{[U]} \geq 0$. From a Weiss sieve $\mathscr{U}$ of $M$ we easily get a Weiss sieve $\mathscr{U}_{[U]}$ of $M_{[U]}$ by intersecting with $M_{[U]}$ as a submanifold of $M$. Using $\mathscr{U}_{[U]}$, we define $\mathscr{U}_{i_{[U]}, [U]}$ to be the sieve of $\mathrm{Conf}_{i_{[U]}}(M_{[U]})_{\Sigma_{i_{[U]}}}$ as in \Cref{lem:weiss_to_ordinary}. These considerations give us equivalences
    %
    \begin{align}\label{eq:ord_to_W_equivs}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\mathrm{Conf}_{i_{[U]}}(-_{[U]})_{\Sigma_{i_{[U]}}}} \mathsf{Sp}) &\simeq \mathsf{colim} (\mathscr{U}_{[U]} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\mathrm{Conf}_{i_{[U]}}(-)_{\Sigma_{i_{[U]}}}} \mathsf{Sp})\notag\\
        &\simeq \mathsf{colim} (\mathscr{U}_{i_{[U]},[U]} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{} \mathsf{Sp}).
    \end{align}
    %
    The second equivalence in the above is simply a statement about the unions over the two covers and is a restatement of \Cref{lem:weiss_to_ordinary}. But, by \cite[Prop.A.3.2]{lurie_ha} the induced map is an equivalence
    %
    \begin{equation}\label{eq:gluing_top_lurie}
        \mathsf{colim} (\mathscr{U}_{i_{[U]},[U]} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{} \mathsf{Sp}) \xrightarrow{\ \ \simeq \ \ } \mathrm{Conf}_{i_{[U]}}(M_{[U]})_{\Sigma_{i_{[U]}}}.
    \end{equation}
\end{proof}
...
\begin{proof}[Proof of {\Cref{prop:fh_disk_to_lcfa}}]
    ...
    %
    \begin{equation}\label{eq:fh_colim_formula}
        \int_e F \simeq \mathsf{colim} (\disk_{/N} \xrightarrow{\simeq} (\disk_{/M})_{/e} \xrightarrow{} \disk_{/M} \xrightarrow{F} \catC),
    \end{equation}
    %
    and this allows us to rewrite our canonical map from \Cref{eq:fhfunc_canonical_map}. On the right-hand side we just insert \Cref{eq:fh_colim_formula}, but the left-hand side requires more elaboration. After substituting \Cref{eq:fh_colim_formula} into the left-hand side of \Cref{eq:fhfunc_canonical_map} we're doing two colimits, and we can pull some information from the inner colimit into the outer one, namely as
    %
    \begin{equation}
        \mathsf{colim} \Big( \mathscr{U} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\disk_{/-}} \cat| \xrightarrow{\mathsf{colim}(- \xrightarrow{} \disk_{/M} \xrightarrow{F} \catC)} \catC \Big),
    \end{equation}
    %
    where $\cat|$ is the restriction to the image of the functor $\mfld[r]_{/M} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{} \cat$ in the above.... We want to show that $ $
    ...
    \end{align}
    %
    where for the right-hand side we've just inserted \Cref{eq:fh_colim_formula}, while for the left-hand side we've further included a rewriting of the two colimits in play. However, the statement of \Cref{lem:gluing_disk/M} is exactly the statement that the above map is an equivalence.
\end{proof}
...
The above proof already shows how the ideas that went into defining factorization algebras are not that different from the ideas that went into defining factorization homology. Namely, the ideas of Weiss cosheaves and algebras over disks are deeply connected. Now that this connection has been illuminated, the proof of \Cref{thm:disk_alg=lcfa}, which we delve into promptly, is not hard to establish.
