...
In the service of working towards proofs of all of these results we first have to pause and develop some necessary technically results regarding Weiss covers.
...
\begin{lemma}\label{lem:weiss_to_ordinary}
    ...
    \begin{enumerate}
        ...
        \item for all $n \geq 0$, the sieve $\{ \mathrm{Conf}_n(U)_{\Sigma_n} \xhookrightarrow{} \mathrm{Conf}_n(M)_{\Sigma_n} \mid U \in \mathscr{U} \}$ is a covering sieve of the unordered configuration space $\mathrm{Conf}_n(M)_{\Sigma_n}$ of $n$ points in $M$.
    \end{enumerate}
\end{lemma}
...
\begin{lemma}\label{lem:gluing_disk/M}
    A Weiss sieve $\mathscr{U} \subset \mfld[r]_{/M}$ canonically determines a morphism
    %
    \begin{equation}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r] \xrightarrow{\disk_{/-}} \mathsf{RFib}_{\disk(\bstr)}) \xrightarrow{\ \ \simeq \ \ } \disk_{/M}
    \end{equation}
    of right fibrations over $\disk(\bstr)$, which moreover, is an equivalence.
\end{lemma}

\begin{proof}
    The morphism is determined by the universal property of the colimit with the component maps $\disk_{/U} \xrightarrow{} \disk_{/M}$ given from the open embeddings $U \xhookrightarrow{} M$. To show the equivalence, we follow the strategy laid out in part 2 of the proof of \cite[Prop.2.22]{af_primer}, while at the same time adapting it to the stratified setting. A morphism of right fibrations, as in the statement, is an equivalence if, and only if, it is so when looked at as a right fibration over the maximal $\infty$-groupoid of the base (cite ...). In our case this is\footnote{Specifically, the wreath product of groups in the notation is referring to $\mathsf{End}_{\bstr}(U) \wr_{\{1, 2, \dots, i\}} \Sigma_i$, the one whose underlying set is $\mathsf{End}_{\bstr}(U)^{\times \{1, 2, \dots, i\}} \times \Sigma_i$.}
    %
    \begin{equation}
        \disk(\bstr)^{\simeq} \simeq \coprod_{i \geq 0} \prod_{[U]} \mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i),
    \end{equation}
    %
    as given by \cite[Lem.2.21]{aft_fhstrat}. The same lemma also tells us that the maximal $\infty$-groupoid of $\disk_{/M}$ is given by\footnote{The notation $M_{[U]}$ denotes the $[U]$-stratum of the stratified manifold $M$, which is a \emph{smooth} manifold, as defined in \cite[Def.4.4.5]{aft_localstrut}.}
    %
    \begin{equation}
        \disk_{/M}^{\simeq} \simeq \coprod_{i \geq 0} \prod_{[U]} \mathrm{Conf}_{i}(M_{[U]})_{\Sigma_i}.
    \end{equation}
    %
    In both cases, the product runs over isomorphism classes of basics. It is also clear that the map $\disk_{/M}^{\simeq} \xrightarrow{} \disk(\bstr)^{\simeq}$ resulting from the right fibration from the slice respects this decomposition in that there result maps of spaces
    %
    \begin{equation}
        \mathrm{Conf}_i(M_{[U]})_{\Sigma_i} \xrightarrow{\ \ \ \ } \mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i),
    \end{equation}
    %
    for each $i \geq 0$, and each isomorphism class $[U]$ of basics in $\bstr$. Thus, working piece by piece, we need to show that
    %
    \begin{equation}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\mathrm{Conf}_{i}(-_{[U]})_{\Sigma_i}} \mathsf{Sp}_{/\mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i)}) \xrightarrow{\ \ \simeq \ \ } \mathrm{Conf}_{i}(M_{[U]})_{\Sigma_i},
    \end{equation}
    %
    for all $i$ and $[U]$ as specified before, where we notice that the decomposition into maps of spaces means that our target is even $\mathsf{Sp}_{/\mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i)}$ and not just right fibrations. We will first explain this equivalence at the level of spaces and only after will we argue why the equivalence respects the maps to $\mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i)$. From a Weiss sieve $\mathscr{U}$ of $M$ we easily get a Weiss sieve $\mathscr{U}_{[U]}$ of $M_{[U]}$ by intersecting with $M_{[U]}$ as a submanifold of $M$. Using $\mathscr{U}_{[U]}$, we define $\mathscr{U}_{i, [U]}$ to be the sieve of $\mathrm{Conf}_i(M_{[U]})_{\Sigma_i}$ as in \Cref{lem:weiss_to_ordinary}. These considerations give us equivalences
    %
    \begin{align}\label{eq:ord_to_W_equivs}
        \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\mathrm{Conf}_{i}(-_{[U]})_{\Sigma_i}} \mathsf{Sp}) &\simeq \mathsf{colim} (\mathscr{U}_{[U]} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\mathrm{Conf}_{i}(-)_{\Sigma_i}} \mathsf{Sp})\notag\\
        &\simeq \mathsf{colim} (\mathscr{U}_{i,[U]} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{} \mathsf{Sp}).
    \end{align}
    %
    The second equivalence in the above is simply a statement about the unions over the two covers and is a restatement of \Cref{lem:weiss_to_ordinary}. But, by \cite[Prop.A.3.2]{lurie_ha} there is an equivalence
    %
    \begin{equation}\label{eq:gluing_top_lurie}
        \mathsf{colim} (\mathscr{U}_{i,[U]} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{} \mathsf{Sp}) \xrightarrow{\ \ \simeq \ \ } \mathrm{Conf}_{i}(M_{[U]})_{\Sigma_i}.
    \end{equation}
    %
    It is obvious that the equivalences in \Cref{eq:ord_to_W_equivs} lie over $\mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i)$, since they're nothing more than a rewriting. The equivalence in \Cref{eq:gluing_top_lurie} also lies over $\mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i)$ because all the various pieces that we're gluing in $\mathsf{Sp}$ come from unordered configuration space manifolds............. 
\end{proof}

\begin{remark}
    That the maps to $\mathsf{B}(\mathsf{End}_{\bstr}(U) \wr \Sigma_i)$ don't play much of a role can also be seen through the fact that this is the only place where the choice of tangential structure $\bstr$ enters the discussion in the form of including or excluding certain endomorphisms. 
\end{remark}
...
\begin{proof}[Proof of {\Cref{prop:fh_disk_to_lcfa}}]
    ...
    \end{equation}
    %
    If we can find the dashed functor which makes the diagram commute, as indicated above, we would be done with the construction. This amounts to showing that the left Kan extension $\int F$ of a functor $F \in \mathsf{Fun}(\disk_{/M}, \catC)$ along $\disk_{/M} \xhookrightarrow{} \mfld_{/M}$ is automatically a Weiss cosheaf. By the definition of $\csheaves[W](\mfld_{/M}, \catC)$, this means that the restriction of $\int F$ to $\mathsf{Fun}(\mfld[r]_{/M}, \catC)$ satisfies the cosheaf property for every Weiss sieve $\mathscr{U} \subset (\mfld[r]_{/M})_{/e}$, where $e:N \xhookrightarrow{} M$ is an open embedding. Here we notice that for the Weiss property to even make sense, we are implicitly using \Cref{lem:double_slice}, so that the Weiss sieve $\mathscr{U}$ is defined as a subcategory of $\mfld[r]_{/N}$.

    A similar calculation appears in part 2 of the proof of \cite[prop.2.22]{af_primer}, with the caveat that there things are presented for smooth manifolds. However, because of \Cref{lem:weiss_to_ordinary} and \Cref{lem:gluing_disk/M} we can adapt the proof to the stratified setting as well. Namely, we're trying to show that the canonical map 
    %
    \begin{equation}\label{eq:fhfunc_canonical_map}
        \mathsf{colim}(\mathscr{U} \xhookrightarrow{} \mfld[r]_{/N} \xrightarrow{} \mfld[r]_{/M} \xrightarrow{} \mfld_{/M} \xrightarrow{\int F} \catC) \xrightarrow{\ \ \ \ } \int_e F
    \end{equation}
    %
    is an equivalence for each sieve $\mathscr{U}$. Since $\catC$ has all relevant colimits the left Kan extension $\int F$ is given pointwise by
    %
    \begin{equation}
        \int_e F \simeq \mathsf{colim} (\disk_{/N} \xrightarrow{\simeq} (\disk_{/M})_{/e} \xrightarrow{} \disk_{/M} \xrightarrow{F} \catC),
    \end{equation}
    %
    and this allows us to rewrite our canonical map from \Cref{eq:fhfunc_canonical_map} into
    %
    \begin{align}
        \mathsf{colim} \Big( \mathsf{colim} (\mathscr{U} \xrightarrow{} \mfld[r](\bstr) \xrightarrow{\disk_{/-}} \cat) \xrightarrow{} &\disk_{/N} \rightarrow \disk_{/M} \xrightarrow{F} \catC \Big)\notag\\
        \xrightarrow{\ \ \ \ \ } \mathsf{colim} (&\disk_{/N} \xrightarrow{} \disk_{/M} \xrightarrow{F} \catC).
    \end{align}
    %
    However, the statement of \Cref{lem:gluing_disk/M} is exactly the statement that the above map is an equivalence.
\end{proof}
...
\begin{remark}
    As noted at the beginning of \S 2, the physical interpretation of this version of factorization homology is a way to take a spacetime $M$ and its local observables $A$, and construct \emph{some}, and typically not all, of the global observables. The previous \Cref{thm:disk_alg=lcfa} and \Cref{cor:disk_alg=falg} transfer this observation to factorization algebras, which aligns with the observation of \cite{cg2016} that the current version of factorization algebras can only deal with perturbative quantum field theory. 
\end{remark}
