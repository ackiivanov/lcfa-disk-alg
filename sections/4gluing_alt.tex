...
After a simplification down to disks, in the locally constant case, lemma \Cref{lem:disk/M_to_disk} points towards a result like the following:

%\begin{conjecture}
%    The theory of factorization algebras is fully determined, using factorization homology, by the data of a functor
%    %
%    \begin{equation}
%        \mathsf{Bsc} \xrightarrow{\ \ \ \ } \cat^{\mathsf{op}}.
%    \end{equation}
%    %
%    In the locally constant case, we can even say that to each basic $U$ this functor attaches the $\infty$-category $\mathsf{Fun}^{\otimes} (\disk(\mathsf{B}_U), \catC)$ of disk algebras with $\mathsf{B}_U$ structure, where $\mathsf{B}_U$ is the sieve poset whose maximal element is $U$.
%\end{conjecture}

%\begin{remark}
%    The $\infty$-category of basics $\mathsf{B}_{U}$ for our purposes provides a kind of generalization of framing, which makes calculation easier.
%\end{remark}

%\begin{proof}
%    The second part of the statement simply concerns what data defines a locally constant factorization algebra on a disk. 
%\end{proof}
...
%    %
%    \begin{equation}
%        \lcfa_{-}(\catC): \mfld \xrightarrow{\ \ \ \ } \cat^{\mathsf{op}}.
%    \end{equation}
%\end{proposition}
...
%    \end{equation}
%    %
%    where the morphism space notation was introduced in \Cref{def:mfld_infty_cat}. Any map from a space will factor through the maximal $\infty$-groupoid of the codomain, so that we can drop the $^{\simeq}$ requirement. Unraveling the definition of a functor category at the same time means that we need to provide maps
%    %
%    \begin{equation}
