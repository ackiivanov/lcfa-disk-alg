...
\end{equation}
%
where the action on morphisms is given by restriction, which is what gives rise to the $^{\mathsf{op}}$. 


\begin{proposition}
    Factorization algebras can be formulated as functors
    %
    \begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    %
    where the morphism space notation was introduced in \Cref{def:mfld_infty_cat}. Any map from a space will factor through the maximal $\infty$-groupoid of the codomain, so that we can drop the $^{\simeq}$ requirement on the target. Unraveling the definition of a functor category at the same time means that we need to provide maps
    %
    \begin{equation}
    ...
    \begin{align}
        ...
        \xrightarrow{\mathrm{restrict}}& \mathsf{Hom_{sSet}} (\mathscr{F}(N \times \Delta^p), F(M \times \Delta^p)) \\
        \xrightarrow{\ \pi_{1*} \circ -}& \mathsf{Hom_{sSet}} (F(N \times \Delta^p) , F(M)) \\
        \xrightarrow{\ - \circ \alpha^p \ }& \mathsf{Hom_{sSet}} (F(N) \times \Delta^p , F(M))
    \end{align}
...
In the case of disjoint union, \Cref{thm:gluing_lcfas} and \Cref{cor:gluing_falgs} then provide the equivalences that are needed to make this into a symmetric monoidal functor, where the symmetric monoidal product on $\cat^{\mathsf{op}}$ is given by \emph{coproduct}. With that understanding the same two results, \Cref{thm:gluing_lcfas} and \Cref{cor:gluing_falgs}, can be understood as saying that the functors $\falg(\catC)$ and $\lcfa(\catC)$ satisfy $\otimes$-excision as outlined in \Cref{sec:excision_homology}.% We can even work a bit more to show:
%
%\begin{proposition}
...
%
%    An object of the limit is a collection $(A_0, A_1, \dots)$ of factorization algebras on $M_0, M_1, \dots$, respectively, together with equivalences $\eta_i: A_{i+1}|_{M_i} \simeq A_i$ for $i \geq 0$.
%\end{proof}
