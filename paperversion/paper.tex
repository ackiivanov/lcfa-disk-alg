...
\end{lemma}
\begin{proof}
    Up to isomorphism we have that $U=C^{<1}(L)\subset C^{<1}(Z)\subset C(Z)$ is induced by the inclusion $L=S_Z\subset Z$. Abbreviating $S=S_{C(Z)}$, the inclusion of $U$ factors as $U\subset S=C(L)$ . Now, using \Cref{7GBVGBA} we have $\Unzip C(Z)=\Unzip_{S}C(Z)=\R_{\geq0}\times\Unzip_L Z$ and so 
    \begin{align*}
